            ...
            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                ...
                <form class="space-y-4 md:space-y-6" @submit.prevent="loginForm">
                    ...
                    <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                        Don’t have an account yet? <a @click="registerUser" class="font-medium text-primary-600 hover:underline dark:text-primary-500 cursor-pointer text-white">Sign up</a>
                    </p>
                </form>
            </div>
        </div>
...
export default {
  ...
  methods: {
    ...mapActions(useCounterStore, ['signInUser']),
    ...mapActions(useCounterStore, ['googleSignInUser']),
    registerUser(){
    ...
    async loginForm(){
        try {
            ...
            await this.signInUser(value)
        } catch (err) {
            console.log(err);
        }
    },
    callback(response) {
      this.googleSignInUser(response.credential)
    },
  }
}
</script>
